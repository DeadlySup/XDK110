...
static xTimerHandle bleTransmitTimerHandle;
...
/* buffer for data receive function */
	...
	{
		ReturnVal = sprintf((char*) Buffer, "%ld %ld %ld", (long int) Data[a], (long int) Data[a+1], (long int) Data[a+2]); /* keep transmitting data in blocks of 4 until array is cmpletely transmitted */
	...
	switch (bleRetval)
	{
		...
		case RETCODE_BLETRANSPORT_UNKNOWN_ERROR: printf("An unknown error occured. \r\n");
			break;
		case RETCODE_OK: printf("Everything's going well. \r\n");
			 break;
...
static void BleAccelDataSentCallback(Retcode_T sendStatus)
{
    ...
    if ( xSemaphoreGive( SendCompleteSync ) != pdTRUE)
    {
    	...
    	a = 0;
        /*We would not expect this call to fail because we must have obtained the semaphore to get here.*/
        Retcode_RaiseError(RETCODE(RETCODE_SEVERITY_ERROR, SEMAPHORE_GIVE_ERROR)); /*this is the given error */

    }
    else
    {
    	if (pdPASS != xTimerStart(bleTransmitTimerHandle, 0))
    	{
    		exit(0);
    	}
	}
	...
	if (retval == RETCODE_OK)
		{
			...
			{
				Data[i] = DataStr.temperature-4000; /*Self-heating offset specified as 3-4K */
...
Retcode_T init(void)
{
    ...
    bleTransmitTimerHandle = xTimerCreate((char * const ) "bleTransmitTimerHandle", MS(100), TIMER_AUTORELOAD_OFF, NULL, EnqueueAccelDatatoBLE);
    if (RETCODE_OK == retval && retval2 == RETCODE_OK)
    {
    	...
    	{
			EnvironmentalTimerHandle = xTimerCreate((char * const) "eTimerHandle", 2000, pdTRUE, NULL, EnvironmentalMeasure); /* should be ten minutes */
